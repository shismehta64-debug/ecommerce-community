...
import { Search, MapPin, ShoppingBag, Plus, ShoppingCart, Heart } from 'lucide-react';
import Pagination from '../../components/Pagination';
import Badge from '../../components/Badge';
import { WomenProductCategory } from '../../types';
...
export default function WomenStore() {
  ...
  const { data: storeData, isLoading } = useQuery({
    queryKey: ['womenProducts', search, category, city, page],
    queryFn: () => listProducts({ search, category, city, page, limit: 12 }),
  });
  ...
  const handleAddToCart = async (e: React.MouseEvent, item: any) => {
    ...
    if (currentQty + 1 > item.stockQuantity) {
      addToast(`Cannot add more. Only ${item.stockQuantity} items in stock.`, 'error');
      return;
    }
    ...
    try {
      ...
      addToast(`Added 1 item of "${item.name}" to cart`, 'success');
    } catch (err: any) {
  ...
  const categories: WomenProductCategory[] = [
    'HANDICRAFTS',
    'FOOD_PICKLES',
    'CLOTHING_BOUTIQUE',
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">Women Entrepreneur Store</h1>
          <p className="text-sm text-slate-500 mt-1">
            Shop handmade crafts, home-made Gujarati spices/pickles, designer wear, and jewelry from local women creators.
          </p>
        </div>
        <Link
          to="/women/new"
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 bg-white rounded-3xl border border-slate-100 shadow-sm">
          ...
          <p className="text-slate-400 text-sm mt-1">Try resetting your filters or posting a new product listing.</p>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
                ...
                <div>
                  ...
                  <div className="p-5 space-y-2">
                    <div className="flex items-center justify-between">
                      <Badge variant="info">{item.category.replace('_', ' ')}</Badge>
          ...
          <Pagination
            currentPage={storeData.meta.page}
            totalPages={storeData.meta.totalPages}
