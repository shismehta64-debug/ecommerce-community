...
import { register as registerApi, loginWithGoogle } from '../api/auth';
import { Loader2, Mail, Phone, Lock, User as UserIcon, MapPin } from 'lucide-react';
...
export default function Register() {
  ...
  const handleGoogleSuccess = async (credentialResponse: any) => {
    ...
    try {
      ...
      addToast(`Account created successfully! Welcome, ${result.user.fullName}.`, 'success');
      navigate('/dashboard');
    } catch (error: any) {
      ...
      <div className="max-w-md w-full space-y-8 bg-white p-8 sm:p-10 rounded-3xl border border-slate-100 shadow-md">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-slate-500">
            Join the community connect platform and get access
          </p>
        </div>
        ...
        <form className="mt-8 space-y-4" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Gujarat"
                {...formRegister('state')}
                className="w-full px-4 py-3 rounded-xl border border-slate-200 text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-sm"
              />
              {errors.state && (
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full flex justify-center items-center py-3 px-4 border border-transparent text-sm font-bold rounded-xl text-white bg-primary hover:bg-indigo-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow mt-6"
          >
            {isLoading ? (
              <>
